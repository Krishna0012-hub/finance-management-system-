	...
	let message = "";
	 $.each(account_list_arr, function(key, value) {
       ...
       $('#account_data_table').prepend('<tr id="row_id_0"><td></td>\
                                        ...
                                        <td><input type="text" id="diff_amount" style="width:80px"/></td>\
                                        <td></td>\
	                                    <td></td>\
									<td><button type="button" id="save_acc_data" onclick="save_acc_data(0)" /><i class="fas fa-save"></i></button>\
									<button type="button" id="cancel_button"  onclick="cancel_row(0)" /><i class="fas fa-times"></i></button></td>\
                                    <tr>');
...
function update_account(id){
	...
	$("#row_id_"+id).find('td').each (function() {
		...
		if(i==2){
			...
			html += '<td><input type="text" id="fixed_amount" style="width:80px" value= "'+td_val+'"/></td>';
		}else if(i==6){
			html += '<td><input type="text" id="veriable_amount" style="width:80px" value= "'+td_val+'"/></td>';
		}else if(i==7){
			html += '<td><input type="text" id="diff_amount" style="width:80px" value= "'+td_val+'"/></td>';
		}else if(i==8){
			html += '<td></td>';
		}else if(i==9){
			html += '<td></td>';
		}else if(i==10){
			html += '<td><button type="button" id="save_acc_data"  onclick="save_acc_data('+id+')" /><i class="fas fa-save"></i></button>\
						<button type="button" id="cancel_button"  onclick="cancel_row('+id+')" /><i class="fas fa-times"></i></button></td>';
		}else{
			html += '<td></td>';
		}
		i++;
